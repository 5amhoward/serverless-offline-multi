...
exports.start = async function () {
    ...
    const serviceCommands = determinedArgs.map((dirAndPort) => {
        ...
        return {
            command: `cd ${process.cwd()}/${directory} && ${installedPath}/nodemon/bin/nodemon.js --exec "serverless offline ${!!port ? `--port ${port} --httpPort ${port} --lambdaPort ${Number(port) + 1000}` : ''}" -e "js,yml"`,
            name: directory,
